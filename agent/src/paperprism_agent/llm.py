...
either OpenAI, Ollama's ``/v1``, Azure, OpenRouter, etc. This keeps the
classifier decoupled from provider nuance.
...
Design notes:
  ...
  - We require JSON output; caller asks for `response_format={"type":"json_object"}`.
    Some gateways (e.g. older Ollama builds) ignore it, so the classifier
...
class LLMClient:
    ...
    def chat_json(
        ...
        *,
        ...
        We ask for JSON mode; callers parse the string themselves so they
        can recover from providers that don't honour `response_format`.
        """
